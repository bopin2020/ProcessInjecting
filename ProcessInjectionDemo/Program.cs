...
namespace ProcessInjectionDemo
{
    class Program
    {
        ...
        {
            #region Doppelganging

            // Doppelganging.Run(args);
            // ProcessDoppelganging.Run();
            ...
            #region SetWindowHookEx
            // _SetWindowHookEx.Run();
            #endregion

            #region Suspend_Thread NO_ACCESS
            SuspendThread_NOAccess.Run();
            #endregion

            // Exception
            // RunPE.Execute(@"c:\users\James\Desktop\MessageBox.exe", File.ReadAllBytes(@"c:\users\James\Desktop\MessageBox.exe"));

            //int length;
            //ProcessGhosting.buffer_payload(@"c:\users\James\Desktop\MessageBox.exe", out length);
    ...
    /* 
        ...
        https://github.com/NYAN-x-CAT/AsyncRAT-C-Sharp      AsyncRAT 作者
    */
